...
| Installed-app launch smoke | iPhone passed / Watch pending | `devicectl` launched `LetItRide.BikeComputer` successfully on the installed iPhone candidate. Two Watch launch attempts timed out establishing the network tunnel after its successful install/version query; open the Watch app manually before counting Watch launch, 2026-07-20. |
...
| App Store Connect privacy-policy URL | Configured / pending branch publication and final verification | App Store Connect app-info localization already points to the documented GitHub URL. The current public `main` policy still predates the Watch/HealthKit and 30-day retention text, so the candidate policy must be published before final tap and submission verification. App Privacy publish state is not exposed by the available public API and remains pending. |
