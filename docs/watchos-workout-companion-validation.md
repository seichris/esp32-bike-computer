...
| Candidate code commit | Pending the local checkpoint for the LVGL-memory and offline-map-onboarding fixes. Prior candidate `5b8ff66232999d7ad5f4ce81ab094d7b5fae4484` (`1.1 (6)`) is superseded and remains historical evidence only. |
| Signed-archive provenance | Pending a new archive from the next exact code checkpoint. `/tmp/open-bike-watch-companion-1.1-6-5b8ff662-20260720.xcarchive` is the superseded `5b8ff662` behavior artifact and does not contain the current onboarding fix. |
| iPhone build | Superseded `5b8ff662` build `1.1 (6)` passed strict signature validation, installation, and launch smoke on an iPhone 16 Pro Max running iOS 26.5.2. Rebuild and reinstall the next exact candidate before counting further physical evidence. |
| Watch build | Superseded `5b8ff662` embedded build `1.1 (6)` passed strict signature validation and installation on an Apple Watch Series 8 running watchOS 26.5, and the user manually opened it. Rebuild and reinstall the containing app from the next exact candidate before counting further physical evidence. |
| ESP32 1.75 firmware SHA | Pending a clean-commit rebuild and flash. Working-tree diagnostic binaries proved the LVGL exhaustion root cause and fix but are superseded by the reviewed source delta. |
| ESP32 2.06 firmware SHA | Pending a clean-commit rebuild. |
...
| Installed-app launch smoke | Passed on iPhone and Watch | `devicectl` launched `LetItRide.BikeComputer` successfully on the installed iPhone candidate. After automated Watch launch attempts were blocked by the paired-device tunnel, the user manually opened the exact installed Watch app and confirmed it was open, 2026-07-20. |
...
| Existing navigation/BLE regression tests | Passed on current source / pending checkpoint binding | `./scripts/run-navigation-tests.sh`, including the installed-map onboarding regression, passed on the reviewed working-tree source, 2026-07-20. Rerun after the exact code checkpoint. |
| `WAVESHARE_AMOLED_175` firmware build, flash, and boot | Working-tree root cause and fix proved / pending clean checkpoint | A temporary task-watchdog build captured LVGL's software-image renderer looping after its built-in 64 KiB allocator returned null. Raising the pool to 96 KiB let the authenticated iPhone connection transition into the configured `MAP_GUIDANCE` screen and continue LVGL/display heartbeats through 42 seconds and a reconnect. The tested working-tree binary is superseded; rebuild and flash from the exact code checkpoint before promoting this row. The existing GPIO21 hint-plus-fallback touch policy remains unchanged. |
| `WAVESHARE_AMOLED_206` firmware build | Passed on current source / pending checkpoint binding | The reviewed working-tree source built successfully at RAM 50.8% and flash 88.5% (2,784,739 / 3,145,728 bytes), 2026-07-20. Rebuild after the exact code checkpoint. |
