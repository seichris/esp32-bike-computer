...
void BLENavigationServer::beginSimulationSession() {
  if (!simulationSessionActive) {
    ...
    stats.sessionNavPacketCount = 0;
    stats.sessionRoutePacketCount = 0;
    stats.sessionRouteDuplicateCount = 0;
    stats.sessionGpsPacketCount = 0;
    stats.sessionSettingsPacketCount = 0;
...
void BLENavigationServer::handleRouteWrite(const uint8_t *data, uint16_t len,
  ...
  if (bike_ble::isDuplicateRouteSummary(summary, lastRouteHash, lastRouteLen)) {
    stats.routeDuplicateCount++;
    stats.sessionRouteDuplicateCount++;
    stats.lastRouteDuplicateMs = millis();
    stats.lastRoutePacketMs = stats.lastRouteDuplicateMs;
    refreshStats();
    Serial.print("BLE: ");
    Serial.print(source == nullptr ? "unknown" : source);
    Serial.print(" duplicate route ignored hash=0x");
    Serial.print(summary.hash, HEX);
    Serial.print(" bytes=");
    Serial.println(summary.lengthBytes);
    return;
  }
...
void BLENavigationServer::handleSettingsWrite(const uint8_t *data, uint16_t len,
  ...
  stats.sessionSettingsPacketCount++;
