...
bool MapLite::begin() {
  ...
  Serial.print(sdReady ? "ready" : "unavailable");
  Serial.print(" experiment=");
  Serial.println(currentStatus.experimentEnabled);
  return sdReady;
}

bool MapLite::updateForGps(int32_t latMicrodegrees, int32_t lonMicrodegrees,
                           uint32_t nowMs) {
  if (!currentStatus.experimentEnabled || !sdReady) {
    return false;
  }
...
bool MapLite::renderLastProbePreview(DisplayRound &display, uint32_t nowMs) {
  if (!currentStatus.experimentEnabled || !currentStatus.hasProbe ||
      !currentStatus.lastResult.found ||
  ...
  if (sdReady && file.open(currentStatus.lastResult.path, O_RDONLY)) {
    ...
    if (header.headerValid &&
        renderPolygonRecords(file, display, header, featureCount, segmentCount,
                             skippedSegmentCount, budgetExceeded)) {
...
void MapLite::setExperimentEnabled(bool enabled) {
  if (currentStatus.experimentEnabled == enabled) {
    Serial.print("MapLite: experiment already ");
    Serial.println(enabled ? "enabled" : "disabled");
    return;
  }
  currentStatus.experimentEnabled = enabled;
  Serial.print("MapLite: experiment ");
  Serial.println(enabled ? "enabled" : "disabled");
}
...
bool MapLite::renderPolygonRecords(SdFile &file, DisplayRound &display,
  ...
  for (uint16_t i = 0; i < result.polygonCount; i++) {
    ...
    const bool renderCandidate =
        map_lite_core::isCandidatePolygon(result.version, typeId);
    ...
    int16_t previousX = 0;
    int16_t previousY = 0;
    if (!readI16(file, firstX) || !readI16(file, firstY)) {
      return false;
    }
    previousX = firstX;
    previousY = firstY;
    ...
    if (pointCount > 2) {
      if (featureCount < map_lite_core::RENDER_FEATURE_BUDGET &&
        ...
        display.drawLine(map_lite_core::localMapCoordToScreen(previousX, false),
                         map_lite_core::localMapCoordToScreen(previousY, true),
                         map_lite_core::localMapCoordToScreen(firstX, false),
                         map_lite_core::localMapCoordToScreen(firstY, true),
                         color);
        segmentCount++;
        featureDrewSegment = true;
      } else {
