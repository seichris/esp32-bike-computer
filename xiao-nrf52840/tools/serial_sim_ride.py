...
def emit_commands(args: argparse.Namespace) -> Iterable[str]:
    ...
    route_payload_hex = route_hex(points)
    ...
    yield "SIM ON"
    yield f"SET {BRIGHTNESS_SETTING_ID} 80"
    yield "SET 7 3"
    yield "SET 6 1"
    yield "NAV 1|400|Continue on test route"
    yield f"ROUTEHEX {route_payload_hex}"
    if args.duplicate_route:
        yield f"ROUTEHEX {route_payload_hex}"
    ...
    for sample in range(sample_count + 1):
        ...
        if sample > 0 and elapsed % args.setting_period == 0:
            ...
            yield f"SET {BRIGHTNESS_SETTING_ID} {60 + ((setting_cycle % 3) * 10)}"
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--duplicate-route",
        action="store_true",
        help="emit the initial route geometry twice to exercise duplicate debounce",
    )
