...
- Battery-divider assumption: 2:1, persisted as `battery_scale_permille=2000`.
  This must be calibrated against a multimeter before battery percentage or
  low-battery thresholds are trusted.
...
- `BATCAL measured_mV` value used, resulting `battery_scale_permille`, and the
  next diagnostic `battery_mv` reading.
- `tools/serial_log_check.py --profile power-rtc-calibration` result for the
  captured log.
...
Add one measured runtime evidence line to the captured serial log:

```text
EVIDENCE runtime_minutes=95 thermal_max_c=42 start_battery_mv=4120 end_battery_mv=3480 brightness_pct=80
```

Then validate it together with calibrated battery diagnostics:

```sh
python3 xiao-nrf52840/tools/serial_log_check.py /tmp/xiao-power-runtime.log \
  --profile power-runtime
```

The `power-runtime` profile requires a successful `BATCAL` before the passing
diagnostic snapshot, plausible battery voltage/scale diagnostics, measured
runtime of at least 30 minutes, a battery-voltage drop of at least 50 mV,
`brightness_pct` in the usable 5..100 range, `thermal_max_c <= 50`, and
`start_battery_mv`/`end_battery_mv` that match the captured diagnostic
...
5. Disconnected idle recovery: wait until backlight-off, then wake by touch and
   by iOS reconnect. Capture `DIAG` while `screen_off=1` after an idle delay,
   capture another `DIAG` after wake/reconnect, and validate the log with
   `tools/serial_log_check.py --profile power-screen-off-recovery`. The
   post-wake snapshot must show `connect_count` and `auth_successes` increased
   beyond the screen-off snapshot.
...
- A captured runtime log passes `tools/serial_log_check.py --profile
  power-runtime`.
- A captured screen-off recovery log passes `tools/serial_log_check.py
  --profile power-screen-off-recovery`.
